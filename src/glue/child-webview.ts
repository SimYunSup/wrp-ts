...
export interface CreateChildWebViewSocketConfig {
  child: WebView;
  onClosed?: () => void;
}
export async function createChildWebViewSocket(
  config: CreateChildWebViewSocketConfig,
): Promise<Closer & Socket> {
  const { child, onClosed } = config;
